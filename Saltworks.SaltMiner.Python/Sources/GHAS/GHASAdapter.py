...
class GHASAdapter:
        ...
        *,
        ...
        The three callers:
          ...
          Real close event    (gate=False, replace=True, watermark_advance set):
            ...
            a closed state. Replace now (bypass the gate) and advance the
            watermark so the same event does not re-trigger.
    ...
    def map_scan(self, repo: dict, engine: str, report_id: str, scan_date: str,
        ...
        # this scope and replaces them with the queued set. True only when we
        # have an authoritative picture to replace with: alerts-present Phase 2,
        # or a CONFIRMED removal (FIX-001 open->closed via prior watermark, or an
        # archived tombstone). False for a plain clean-scan heartbeat — a
        # heartbeat is a visibility tick, not proof the scope is empty. Setting
        # it True on a heartbeat caused progressive data loss: any scope
        # momentarily showing zero OPEN alerts (open-only Phase 2) had its real
        # issues deleted by the empty replacement, run after run.
