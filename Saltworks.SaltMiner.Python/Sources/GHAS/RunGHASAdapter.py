...
Discovers all enabled GHAS instance configs in `Config/Sources/` (any .json
file where `Source == "GHAS"` and `Enabled != false`), then runs each
...
Usage:
    ...
    python RunGHASAdapter.py --config-dir Config/Sources
...
def main():
    ...
    parser.add_argument(
        "--config-dir",
        default="Config/Sources",
        help="Directory containing GHAS instance configs (default: Config/Sources)",
    )
    parser.add_argument(
        "--log-level",
        default="INFO",
