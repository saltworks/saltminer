...
# Written by Saltworks Security, LLC (www.saltworks.io), 2025
...
class SmDataClient(object):
    ...
    def __init__(self, appSettings, sourceName, configName="SMv3"):
        ...
        logging.debug(f"[SMDataClient] SmApiClient initialization complete.  RestClient params - url: '{baseurl}', verify: {verify}, role: {self.Role}, agentID: {self.AgentId}")
    ...
    def AddQueueAsset(self, qAsset):
        ...
        self.__VerifyResponse("[SMDataClient] Error submitting new queue asset to API", r)
        return json.loads(r.text)['data']
    ...
    def DeleteAsset(self, assetId, sourceType):
        ...
        self.__VerifyResponse(f"[SMDataClient] Error deleting asset with ID '{assetId}', source type '{sourceType}'", r)
        logging.debug("[SMDataClient] Deleted asset with ID '%s', source type '%s'", assetId, sourceType)
    ...
    def DeleteScan(self, scanId, sourceType):
        ...
        self.__VerifyResponse(f"[SMDataClient] Error deleting scan with ID '{scanId}', source type '{sourceType}'", r)
        logging.debug("[SMDataClient] Deleted scan with ID '%s', source type '%s'", scanId, sourceType)
    ...
    def DeleteScanIssues(self, scanId, sourceType):
        ...
        self.__VerifyResponse(f"[SMDataClient] Error deleting issue(s) for scan with ID '{scanId}', source type '{sourceType}'", r)
        logging.debug("[SMDataClient] Deleted issue(s) for scan with ID '%s', source type '%s'", scanId, sourceType)
    ...
    def GetAssetScans(self, sourceId, sourceType, refreshFirst=False):
        ...
        self.__VerifyResponse(f"[SMDataClient] Error searching for scans for asset with source ID '{sourceId}' and source type '{sourceType}'", r)
        r = json.loads(r.text)
    ...
    def GetIssues(self, sourceType, query_body= False, refreshFirst=False):
        '''
        Retrieves a list of scans by parameters provided.
        NOTE: requires Manager API Key (ManagerApiKey setting) to function.
        '''
        if query_body:
            ...
            q = {
              ...
              "uiPagingInfo": {
                "size": 1000,
                "sortFilters": {
                  "vulnerability.found_date": False
                }
              }
            }

        if refreshFirst:
            ind = f"issues_app_saltworks.{sourceType.lower()}_{self.__SourceName.lower()}"
            self.RefreshIndex(ind)
        r = self.__Client.Post("issue/search", q, headers=self.__ManagerHeaders)
        self.__VerifyResponse(f"[SMDataClient] Error loading issues", r)
        r = json.loads(r.text)
        if 'data' in r.keys() and len(r['data']):
            return r['data']
        else:
    ...
    def DeleteAssetAll(self, assetId, sourceId, sourceType, exceptionOnFail=False):
        ...
        try:
            ...
            while scans:
                logging.info("[SMDataClient] Found %s scan(s) to delete in this pass.", len(scans))
                c = 0
                for scan in scans:
                    scanId = scan['id']
                    ...
                    try:
                        ...
                        logging.info("[SMDataClient] Unable to delete issues for scan ID '%s' and source type '%s'", scanId, sourceType)
                        if exceptionOnFail:
                            raise SmApiClientException("[SMDataClient] Error deleting issues, see log for details") from e
                        ok = False
                    ...
                    try:
                        ...
                        logging.info("[SMDataClient] Unable to delete scan with ID '%s' and source type '%s'", scanId, sourceType)
                        if exceptionOnFail:
                            raise SmApiClientException("[SMDataClient] Error deleting scan, see log for details") from e
                        ok = False
                    c += 1
                    if c % 50 == 0 or c == len(scans):
                        logging.info("[SMDataClient] Deleted %s of %s scans", c, len(scans))
                # end for
            ...
            try:
                ...
                logging.info("[SMDataClient] Asset '%s' not found, cannot delete.", assetId)
            except Exception as e:
                #logging.warning("[SMDataClient] Unable to delete asset with ID '%s' and source type '%s'", sourceId, sourceType)
                if exceptionOnFail:
                    raise SmApiClientException("[SMDataClient] Error deleting asset, see log for details") from e
                ok = False
            if cancel:
                raise KeyboardInterrupt()
        except KeyboardInterrupt:
            ...
            logging.exception("[SMDataClient] Error processing removal of asset, scan, or issues for asset ID '%s' and source type '%s'", sourceId, sourceType)
            if exceptionOnFail:
                raise SmApiClientException("[SMDataClient] Error deleting asset/scan/issues, see log for details") from e
            ok = False
        finally:
            return ok
    ...
    def FinalizeQueue(self, qScanId):
        ...
        self.__VerifyResponse(f"[SMDataClient] Error finalizing queue scan with ID {qScanId}", r)
        msg = f"[SMDataClient] Completed queuescan with id {qScanId}, now in Pending status"
        logging.info(msg)
    ...
    #     self.__VerifyResponse(f"[SMDataClient] Error searching for last scan for app version {avid} and assessment type '{atype}'", r)
    #     r = json.loads(r.text)
    #     if 'data' in r.keys() and len(r['data']):
    #         return r['data'][0]
    #     else:
    ...
    #     self.__VerifyResponse(f"[SMDataClient] Error returning webhook events for source ID '{sourceId}'", r)
    #     r = json.loads(r.text)
    #     if 'data' in r.keys() and len(r['data']):
    #         return r['data']
    #     else:
    ...
    def __ReadResponseErrors(self, response):
        try:
            ...
            logging.debug("[SMDataClient] Failed to read errors in response with status code %s: [%s] %s", response.status_code, type(e).__name__, e)
    ...
    def __BatchIssue(self, qIssue):
        ...
        if doclen >= self.BatchSize or (not qIssue and doclen > 0):
            msg = f"[SMDataClient] Sending batch of {doclen} queue issue(s) to API"
            logging.debug(msg)
            try:
                r = self.__Client.Post("queueissue/bulk", self.__IssueBatch)
            except Exception as e:
                logging.error("[SMDataClient] Failed sending issue batch to API (%s), retrying in 5 sec...", type(e).__name__)
                r = self.__Client.Post("queueissue/bulk", self.__IssueBatch)
            try:
                self.__VerifyResponse("[SMDataClient] Error sending batch of queue issues to API", r)
            except Exception:
                raise
            finally:
                self.__IssueBatch['Documents'] = []

    def SendAllBatchIssues(self):
        doclen = len(self.__IssueBatch['Documents'])
        if doclen > 0:
            msg = f"[SMDataClient] Sending batch of {doclen} queue issue(s) to API"
            logging.debug(msg)
            try:
                r = self.__Client.Post("queueissue/bulk", self.__IssueBatch)
                logging.info("[SmDataClient] Sent the rest of available batched issues")
            except Exception as e:
                logging.error("[SMDataClient] Failed sending issue batch to API (%s), retrying in 5 sec...", type(e).__name__)
                r = self.__Client.Post("queueissue/bulk", self.__IssueBatch)
            try:
    ...
    #         msg = f"[SMDataClient] Error mapping queue {'asset' if qscan else 'scan'} - [KeyError] for field {e} (section {section}, local id {avid})."
    #         logging.error(msg)
    #         raise SmApiClientException(msg) from e
    #     except SmApiClientException as e:
    #         msg = f"[SMDataClient] Error mapping queue {'asset' if qscan else 'scan'} - [SmApiClientException] {e})."
    #         logging.error(msg)
    #         logging.warning(f"[SMDataClient] Failed to add {section} for app / version with local id {avid} and scan with local id {sid}.")
    #         raise SmApiClientException(msg) from e
    #     except Exception as e:
    #         msg = f"[SMDataClient] Error adding {section} for app / version with local id {avid} and scan with local id {sid}: [{type(e).__name__}] {e}"
    #         logging.error(msg)
    #         raise SmApiClientException(msg) from e
    ...
    #         self.AddQueueIssue(qissue)
    #     except KeyError as e:
    #         logging.error("[SMDataClient] Error mapping queue issue - KeyError for field %s (local scan id '%s').", e, avid)
    #         logging.warning(f"[SMDataClient] Scan with local id '%s' may be out of sync with queue scan id '%s'.", avid, qscanId)
    #         raise SmApiClientException("Mapping error") from e
    #     except SmApiClientException as e:
    #         logging.error("[SMDataClient] Error mapping queue issue - [SmApiClientException] %s", e)
    #         logging.warning("[SMDataClient] Scan with local id '%s' may be out of sync with queue scan id '%s'.", avid, qscanId)
    #     except Exception as ex:
    #         scanId = '[unknown]' if not 'scan_id' in issue.keys() else issue['scan_id']
    #         logging.error("[SMDataClient] Error adding issue with local id '%s' to local scan id '%s' (queue scan id %s): [%s] %s", issue['scanner_id'], scanId, qscanId, type(ex).__name__, ex)
    #         logging.warning("[SMDataClient] Scan with local id '%s' may be out of sync with queue scan id '%s'.", avid, qscanId)
    ...
    def __GetScanDate(issue, source, v2SscScan):
        ...
        if not issue:
            ...
            scanDate = SmDataClient.CleanDateString(lastScanDate) if lastScanDate else SmDataClient.CleanDateString(foundDate)
        ...
        if 'SSC' in source.upper():
            scanDate = SmDataClient.CleanDateString(uploadDate) if uploadDate else scanDate
        if not scanDate:
