...
public class DataClient : IDisposable
{
    ...
    public NoDataResponse IndexBulk(JsonDataRequest request, string indexName)
    {
        return CheckRetry(() => ApiClient.Post<NoDataResponse>($"index/bulk/{indexName}", request)).Content;
    }

    public DataResponse<T> IndexSearch<T>(SearchRequest search, string indexName) where T: SaltMinerEntity
    {
        var jSrch = new JsonSearchRequest
        {
            Filter = search.Filter,
            PagingInfo = search.PagingInfo,
            SortKeys = search.SortKeys,
            IncludeConcurrencyInfo = search.IncludeConcurrencyInfo,
