...
# Example: .\run-test.ps1 "EngagementTests.CRUDTest"

param(
    [string]$TestFilter = "EngagementTests.CRUDTest"
)

$ErrorActionPreference = "Stop"

Write-Host "=== SaltMiner Integration Test Runner ===" -ForegroundColor Cyan
...
while ($elapsed -lt $maxWaitTime) {
    try {
        $response = Invoke-WebRequest -Uri "http://localhost:5000/swagger/index.html" -UseBasicParsing -TimeoutSec 1 -ErrorAction Stop
