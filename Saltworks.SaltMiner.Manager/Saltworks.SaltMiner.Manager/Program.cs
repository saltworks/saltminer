...
public static class Program
{
    ...
    private static void RunManager(IConsoleAppHostArgs args)
    {
        ...
        {
            var emsg = new StringBuilder($"[{ex.GetType().Name}] {ex.Message}");
            var cex = ex;
            while (cex.InnerException != null)
            {
                emsg.Append($" --> [{cex.InnerException.GetType().Name}] {cex.InnerException.Message}");
                cex = cex.InnerException;
            }
            if(startLogger == null)
            {
                Console.WriteLine($"Logger not initialized. Manager initialization error: {emsg}");
            }
            else
            {
                startLogger.LogCritical(ex, "Manager initialization error: {Message}", emsg);
            }
        }
    }
