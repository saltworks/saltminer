    ...
    public class EsClientBucketResponse : EsClientResponse, IElasticClientResponse<ElasticClientCompositeAggregate>
    {
        ...
        internal static IElasticClientResponse<ElasticClientCompositeAggregate> BuildResponseBucketAgg(bool isSuccessful, AggregateDictionary aggs)
        {
            ...
            if (aggs != null)
            {
                ...
                {
                    // Assumes bucket aggregates
                    if (a.Value is TermsAggregate<object> termsAgg)
                    {
                        foreach (var bucket in termsAgg.Buckets)
                        {
                            ...
                            foreach (var v in bucket.Aggregations ?? new AggregateDictionary())
                            {
                                if (v.Value is ValueAggregate valueAgg)
                                {
                                    result.Aggregates.Add(v.Key, valueAgg.Value);
                                }
                            }
                            results.Add(result);
                        }
