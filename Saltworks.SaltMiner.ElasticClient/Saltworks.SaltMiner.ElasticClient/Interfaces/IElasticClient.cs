...
public interface IElasticClient
{
    ...
    /// <param name="updateScript">Painless script performing the update, i.e. 'ctx._source.field = params.update;' (or params.update.property - params is the ES params collection name)</param>
    /// <param name="wait">Whether to wait for update to complete before returning</param>
    /// <param name="refresh">Whether to refresh the index after update completes</param>
    ...
    IElasticClientResponse<T> UpdateByQuery<T>(string query, string indexName, string updateScript, bool wait = true, bool refresh = false) where T : SaltMinerEntity;
    /// <summary>
    ...
    /// <returns>A response object containing the requested results and paging info for further results</returns>
    /// <remarks>Not yet clear if supports lucene, but probably does support DSL (json)</remarks>
    IElasticClientResponse<T> UpdateByQuery<T>(UpdateQueryRequest<T> searchRequest, string indexName, bool wait = true, bool refresh = false) where T : SaltMinerEntity;
    /// <summary>
    ...
    /// <summary>
    /// Searches with aggregates
    /// </summary>
    /// <typeparam name="T">SaltMinerEntity type</typeparam>
    /// <param name="agg">Aggregation request</param>
    /// <param name="searchRequest">Search request definition</param>
    /// <param name="indexName">Index to be searched</param>
    /// <returns>Aggregate response</returns>
    IElasticClientAggregateResponse SearchWithAggregates<T>(IElasticClientRequestAggregation agg, SearchRequest searchRequest, string indexName) where T : SaltMinerEntity;
    /// <summary>
