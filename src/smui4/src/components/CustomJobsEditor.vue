<template>
  <div>
    <div class="d-flex justify-space-between align-center mb-4">
      <span class="text-h6">Custom Jobs</span>
      ...
      <v-card>
        ...
        <v-card-text>
          ...
          <v-select
            ...
            label="Script"
            class="mb-4"
            hint="Select a script from /opt/saltworks/saltminer/custom-jobs/"
            persistent-hint
          />
          <v-text-field
        ...
        <v-card-actions>
          ...
          <v-btn
            color="primary"
            :loading="loading"
            :disabled="!form.name || !form.option || !form.schedule"
            @click="handleSave"
          >
            Save
          </v-btn>
...
import { useCustomJobs } from '../composables/useCustomJobs.js'
import CronScheduleBuilder from './CronScheduleBuilder.vue'

const { jobs, primer, scripts, loading, error, fetchJobs, fetchPrimer, fetchScripts, saveJob, deleteJobs } = useCustomJobs()

const headers = [
  { title: 'Name', key: 'name' },
  { title: 'Description', key: 'description' },
  { title: 'Script', key: 'option' },
...
// Jobs whose option is NOT in the primer list are Custom Jobs —
// their option value is the name of a script in the custom-jobs folder.
const filteredJobs = computed(() => {
  return jobs.value.filter((job) => !primerOptionValues.value.has(job.option))
})

// Dropdown populated from the filesystem listing
const scriptItems = computed(() => {
  return scripts.value.map((name) => ({ title: name, value: name }))
})
