...
TEMPLATES_PATH = os.environ.get("REPORT_TEMPLATES_PATH", "/opt/saltworks/saltminer/report-templates/")
...
def list_templates():
    ...
    for name in os.listdir(TEMPLATES_PATH):
        if not name.lower().endswith('.docx'):
            continue
        filepath = os.path.join(TEMPLATES_PATH, name)
...
def template_exists(filename):
    return os.path.isfile(os.path.join(TEMPLATES_PATH, filename))
