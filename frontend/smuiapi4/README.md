...
File-storage paths (custom jobs, SaltMiner jobs, SSL certs, report templates) are now read from the `sys_config` Elastic index under section `general` / subsection `paths`. Defaults under `/opt/saltworks/saltminer/...` are used when no doc is present. Edit the values via **Settings → General → File Storage Paths** in the UI.
...
| `GET /smuiapi4/custom-jobs/scripts` | Lists files in the configured custom-jobs directory (`general/paths/customJobsPath`) |
| `GET/PUT /smuiapi4/settings/general/paths` | Read/write the file-storage path settings |
