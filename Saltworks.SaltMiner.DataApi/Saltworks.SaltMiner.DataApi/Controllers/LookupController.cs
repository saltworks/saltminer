...
[Auth]
[ApiController]
public class LookupController(LookupContext context, ILogger<LookupController> logger) : ApiControllerBase(context, logger)
{
    private LookupContext Context => ContextBase as LookupContext;
    private readonly string LookupIndex = Lookup.GenerateIndex();
    ...
    public ActionResult<DataResponse<Lookup>> Search([FromBody] SearchRequest search)
    {
        ...
        return Ok(Context.Search<Lookup>(LookupIndex, search));
    }
