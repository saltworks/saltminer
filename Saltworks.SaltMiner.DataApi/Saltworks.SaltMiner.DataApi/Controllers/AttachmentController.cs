...
[Auth]
[ApiController]
public class AttachmentController(AttachmentContext context, ILogger<AttachmentController> logger) : ApiControllerBase(context, logger)
{
    ...
    public ActionResult<DataResponse<Attachment>> Search([FromBody] SearchRequest search)
    {
        ...
        return Ok(Context.Search<Attachment>(Attachment.GenerateIndex(), search));
    }
