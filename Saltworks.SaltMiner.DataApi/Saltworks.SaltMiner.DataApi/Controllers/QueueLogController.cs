...
[Auth]
[ApiController]
public class QueueLogController(QueueLogContext context, ILogger<QueueLogController> logger) : ApiControllerBase(context, logger)
{
    private QueueLogContext Context => ContextBase as QueueLogContext;
    private readonly string QueueLogIndex = QueueLog.GenerateIndex();
    ...
    public ActionResult<DataResponse<QueueLog>> Search([FromBody] SearchRequest request)
    {
        ...
        return Ok(Context.Search<QueueLog>(QueueLogIndex, request));
    }
