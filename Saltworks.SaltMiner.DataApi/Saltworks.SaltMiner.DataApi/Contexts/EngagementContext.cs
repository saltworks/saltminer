...
public class EngagementContext(ApiConfig config, IDataRepo dataRepository, IElasticClientFactory factory, ILogger<EngagementContext> logger, AttachmentContext attachmentContext) : ContextBase(config, dataRepository, factory, logger)
{
    private readonly AttachmentContext AttachmentContext = attachmentContext;
    ...
    public NoDataResponse Delete(string id)
    {
        ...
        var request = new SearchRequest("Saltminer.Engagement.Id", id);
    ...
    {
        var engagements = Search<Engagement>(EngagementIndex,
            new SearchRequest 
            { 
                Filter = new Filter
                {
    ...
    public NoDataResponse SetHistoricalIssues(string id, string groupId)
    {
        ...
        var engagements = Search<Engagement>(EngagementIndex, new SearchRequest
        {
            Filter = new Filter
            {
                FilterMatches = new Dictionary<string, string>
        ...
        foreach(var engagement in engagements?.Data?.Where(x => x.Id != id))
        {
            ...
            issueCount += (int) response.CountAffected;
        }
    ...
    private static EngagementStatus IsValidStatus(string status)
    {
        if (Enum.TryParse<EngagementStatus>(status, out var engagementStatus))
        {
