<template>
  <div class="asset-page_wrapper">
    ...
    <div v-if="submitted !== true" class="asset-main_wrapper">
      ...
      <HeadingText label="Add an Inventory Asset" size="2" />
      ...
      <InputCheckbox
        ...
        :checked="inventoryAsset.isProduction || false"
        :size="'default'"
        @input="
              ...
              <template
                ...
                >
                  <InputText
                    ...
                    :label="createAttributeLabel(attribute)"
                    class="text-field_asset"
                    :placeholder="createAttributeLabel(attribute)"
      ...
      <div class="submit-btn_wrap" @click="createInventoryAsset">
        <ButtonComponent
          label="Create"
          theme="primary"
          class="submit-btn"
...
export default {
  ...
  head() {
    return {
      title: `Add Inventory Asset | ${this.pageTitle}`,
    }
  },
  computed: {
    ...mapState({
  ...
  methods: {
    ...
    getPrimer() {
      return this.$axios
        .$get(`${this.$store.state.config.api_url}/inventoryasset/new/primer`)
        .then((r) => {
          ...
          this.attributeCustomizations = r?.data?.inventoryAsset?.attributes.saltminer || []
    ...
    getAttributeValue(attribute) {
      ...
      if (!(name in attributes)) {
        if (attrType === 'date') {
          attributes[name] = val || val || this.formattedAttributeDate
        } else if (attrType === 'select') {
          attributes[name] = val
        } else if (attrType === 'multiselect') {
          attributes[name] = val || []
        } else if (attrType === 'number') {
          attributes[name] = val || null
        } else {
          attributes[name] = val || ''
        }
      }
    ...
    allRequiredFieldsComplete() {
      ...
      const requiredAttributes = this.attributeCustomizations.filter((v) => v.isRequired && Object.entries(this.inventoryAsset.attributes).find(([key, value]) => key === v.name && this.checkRequiredValue(value)))
    ...
    handleUpdateAttribute(attributeName, newVal) {
      this.inventoryAsset.attributes[attributeName] = newVal
    },
    ...
    createInventoryAsset() {
      ...
      Object.keys(attributes).forEach((attr) => {
        ...
        if (sectionName !== attrDef.section) {
          sectionName = attrDef.section;
          sectionValues = attributeSections[sectionName];
          if (sectionValues === undefined) {
            sectionValues = {};
          }
        }
        Object.assign(sectionValues, {[attrDef.name] : attributes[attr]})
        attributeSections[sectionName] = sectionValues;
      })
      const newAsset = {
        ...this.inventoryAsset
      }
      ...
      newAsset.attributes = attributeSections;
