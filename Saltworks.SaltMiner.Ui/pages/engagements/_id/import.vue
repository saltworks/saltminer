<template>
  <div class="import-page_wrapper">
    ...
    <div v-if="submitted !== true" class="import-main_wrapper">
      ...
      <HeadingText label="Import Issues" size="2" />
      <p>
        ...
        JSON file must be in the format of the Engagement Issue and must provide a Default Asset.<br/><br/>
      ...
      <DropdownControl
        ...
        @update="(opt) => setDefaultValue(opt)"
      />
      <p>
        Default asset is the asset to use whenever asset information is missing
        in the import file. If one or more import lines contain invalid asset
        information (or no asset information and the Default Asset is not set)
        then the file import will be canceled.
      </p>
  ...
  methods: {
    getAssets(callback) {
      return this.$axios
        ...
        .catch((error) => {
          this.handleErrorResponse(error, "Error Getting Engagement Assets")
        })
    },
    setDefaultValue(opt) {
    ...
    importIssue() {
      ...
      let url = `${this.$store.state.config.api_url}/issue/import/${this.id}`
      ...
      return this.$axios
        ...
        .then((r) => {
          if (r.data?.isQueued === true) {
            this.addAlert(["The issue import has been placed into queue for processing."], "success", "In Queue")
          }
          else {
            this.$router.push({
    ...
    jsonTemplate() {
      this.$store.dispatch('modules/issues/downloadEngagementTemplate', null, {
        root: true,
      })
    },
   handleAlertClose() {
