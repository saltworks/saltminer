...
if [ -f "$plog" ]; then
  pstep=$(<"$plog") 
  read -p "OK to resume script (y/n)? [y] " ok
  if [ -z "$ok" ]; then ok="y"; fi
  if [ "$ok" != y ]; then 
    echo "Remove $plog to reset script."
	exit 1
  fi
fi
...
if [ "$osname" == "Ubuntu" ] && [[ "$osver" == 24.04* ]]; then os="U"; fi
if [ "$osname" == "Red Hat nterprise Linux" ] && [[ "$osver" == 8* ]]; then os="R8"; fi
if [ "$osname" == "Oracle Linux Server" ] && [[ "$osver" == 8* ]]; then os="R8"; fi
# Intentionally set to R8 rather then R9 so that existing logic doesn't change.
if [ "$osname" == "Rocky Linux" ] && [[ "$osver" == 9* ]]; then os="R8"; fi
...
if [ "$pstep" == 0 ]; then
  ...
  read -p "OK to continue (y/n)? [y] " ok
  if [ -z "$ok" ]; then ok="y"; fi
  if [ "$ok" != y ]; then
    echo "uh oh, bugging out"
    exit 1
  fi
...
#######################################################################################################################
if [ "$pstep" == 1 ]; then
  read -p "Installing SaltMiner prereqs (y/n)? [y] " ok
  if [ -z "$ok" ]; then ok="y"; fi
  if [ "$ok" == y ]; then
    #   Ubuntu
    if [ "$os" == "U" ]; then
    ...
    if [ "$os" == "R8" ]; then
      ...
      # This is a permanent setting that has security implications, needed to enable reverse proxy connections on-box.
      ...
      # https://unix.stackexchange.com/questions/317698/yum-install-http-is-this-safe
      ...
      sudo echo "user nginx;" | sudo tee /etc/nginx/nginx.conf >/dev/null
      sudo cat nginx.conf | sudo tee -a /etc/nginx/nginx.conf >/dev/null
      ...
      echo ""
      echo "Installing yum version lock package."
  ...
  sudo cp ./error502.html /var/www/html

  echo ""
  echo "Package installation complete."
  echo ""
  
  read -p "Prereqs (dotnet, unzip, nginx, pip) installed ok (y/n)? [y] " ok
  if [ -z "$ok" ]; then ok="y"; fi
  if [ "$ok" != y ]; then
    echo "Re-run this script to resume here after fixing the issues."
    exit 1
  fi
  #if [ "$is_docker" = true ]; then
  #  echo "Elasticsearch and Kibana installation will be skipped because Docker is used."
  #  echo "The appropriate theme will have to be manually installed."
  #  pstep=4
  #  esinstalled=1
  #else
  pstep=2
...
if [ "$pstep" == 2 ]; then
  ...
  if [ "$ok" == y ]; then
    ...
    if [ "$os" == "R8" ]; then
      ...
      sudo yum install -y --enablerepo=elasticsearch elasticsearch-$esver
    ...
    if [ "$os" == "U" ]; then
      ...
      esinstalled=1
      sudo apt update
      sudo apt -y install elasticsearch=$esver
    ...
    touch ./info.txt
    echo "elastic user pwd: $pwd" > ./info.txt
    echo "" >> ./info.txt
    echo "kibana_system user pwd: $kpwd" >> ./info.txt
    echo "" >> ./info.txt
    echo "remote_monitoring_user pwd (Used with Fleet): $rmupwd"  >> ./info.txt
...
if [ "$pstep" == 3 ]; then
  ...
  if [ "$ok" == y ]; then
    ...
    if [ "$os" == "R8" ]; then
      ...
      sudo echo "autorefresh=1" >> "$erepo"
      sudo echo "type=rpm-md" >> "$erepo"
      sudo yum install -y --enablerepo=kibana kibana-$esver
      sudo chmod 644 $erepo
    ...
    if [ "$os" == "U" ]; then
      ...
      sudo apt update
      sudo apt -y install kibana=$esver
    ...
    kbthemever=$esver
    kbtheme="SaltminerKibanaTheme-${kbthemever}.zip"
    if test -f "$kbtheme"; then
      ...
      if [ "$os" == "U" ]; then
        ...
        elkver=$(sudo dnf list installed kibana | tail -n1 | awk '{split($0, a, " "); print a[2]}')
      fi
...
if [ "$pstep" == 5 ]; then
  ...
  if [ "$ok" == y ]; then
    ...
    if [ "$ok" == y ]; then
      ...
      sudo mkdir "$smcfg3"/jobmanager/report-templates
    ...
    # NOTE: if script is being executed a second time because of an error then $pwd probably doesn't exist.
    # Set it before running script again.
    ...
    do
      ...
      if [ $total_wait_time -eq $timeout ]; then
        echo -e "\n${yellow}Timeout exceeded while awaiting Kibana readiness. Exiting script.${white}"
        echo -e "Kibana hasn't reported that it is ready. Check Kibana log file or the OS journal.\n"
        exit 1
      else
        sleep 5
        let "total_wait_time+=5"
        echo "Total wait time: ${yellow}${total_wait_time} seconds${white}."
      fi
    done
    ...
    sudo cp kestrel-saltminer-api.service.txt /usr/lib/systemd/system/kestrel-saltminer-api.service
    sudo cp kestrel-saltminer-ui-api.service.txt /usr/lib/systemd/system/kestrel-saltminer-ui-api.service
    sudo cp saltminer-service-manager.service.txt /usr/lib/systemd/system/saltminer-service-manager.service
    sudo cp saltminer-job-manager.service.txt /usr/lib/systemd/system/saltminer-job-manager.service
    
    # These fail in latest Ubuntu - commenting
    #sudo ln -s /usr/lib/systemd/system/kestrel-saltminer-api.service /etc/systemd/system/kestrel-saltminer-api.service
    #sudo ln -s /usr/lib/systemd/system/kestrel-saltminer-ui-api.service /etc/systemd/system/kestrel-saltminer-ui-api.service
    #sudo ln -s /usr/lib/systemd/system/saltminer-service-manager.service /etc/systemd/system/saltminer-service-manager.service
    #sudo ln -s /usr/lib/systemd/system/saltminer-job-manager.service /etc/systemd/system/saltminer-job-manager.service
    
    # This error occurs in some distros when enabling and starting services:
    # "Failed to enable unit: Refusing to operate on alias name or linked unit file"
    # Solution: https://unix.stackexchange.com/questions/271541/enabling-linked-unit-files-in-systemd
    # It's presumed that this error might become more prevalent as distros evolve. This fork is a first step
    # to mitigate. 

    sudo systemctl daemon-reload

    if [ "$osname" == "Rocky Linux" ]; then
      sudo systemctl enable /usr/lib/systemd/system/kestrel-saltminer-api.service
      sudo systemctl start kestrel-saltminer-api
      sudo systemctl enable /usr/lib/systemd/system/kestrel-saltminer-ui-api.service
      sudo systemctl start kestrel-saltminer-ui-api
      sudo systemctl enable /usr/lib/systemd/system/saltminer-service-manager.service
      sudo systemctl start saltminer-service-manager
      sudo systemctl enable /usr/lib/systemd/system/saltminer-job-manager.service
    ...
    # Newer pip versions report 'error: externally-managed-environment' when
    # command below is issued. Solution is to run command with --break-system-packages argument.
    # https://stackoverflow.com/questions/75608323/how-do-i-solve-error-externally-managed-environment-every-time-i-use-pip-3
    # ...unfortunately --break-system-packages is not backwards compatible. An environment variable can be used instead.
